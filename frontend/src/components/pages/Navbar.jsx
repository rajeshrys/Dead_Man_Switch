...
const Navbar = () => {
  ...
  const {activetab, setactivetab, user,dashboard,setdashboard} = useContext(AuthContext)
  const logo = user ? user.username.charAt(0) : null
  ...
  useEffect(() => {
    if(location.pathname === isDashboard){
    setdashboard(true)
  }else{
    setdashboard(false)
  }
  ...
  return ( 
    <nav className='min-h-20 sticky bg-white/80 backdrop-blur-md  left-0 top-0 w-full bg-white/30 shadow-md py-2'>
        ...
        onClick={handlelogoclick}
         className='w-10 relative bg-gray-200 h-10 rounded-full flex items-center justify-center cursor-pointer active:scale-95 bg-purple-600 text-white'>
