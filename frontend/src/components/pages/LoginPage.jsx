...
const navigate = useNavigate()
  ...
  sessionStorage.setItem("logo", logo)
    ...
    }  else if(response.message === 'Error during login' || response.message === 'Invalid Crendentials' || response.message === 'User not found with Email') {
      setmessages(response.message);
      setmess(false)
    }
  ...
  <div className="flex items-center justify-center">
    <div className="sticky top-0 left-0 max-w-60 shadow-[0_2px_4px_rgba(0,0,0,0.04),0_8px_16px_rgba(0,0,0,0.06),0_24px_48px_rgba(0,0,0,0.08)] h-15 flex items-center justify-center px-3 rounded-2xl">
     ...
     <div className='w-full flex p-7 items-center justify-center'>
      <div className='flex min-w-120 min-h-100 flex-col rounded-xl shadow-[0_2px_4px_rgba(0,0,0,0.04),0_8px_16px_rgba(0,0,0,0.06),0_24px_48px_rgba(0,0,0,0.08)]  gap-12'>
        <div className='flex items-center justify-center'>
...
{error && (
  <p className='text-red-600 text-center text-xl'>{error}</p>
            ...
            <div className='flex flex-row gap-2 items-center justify-center w-full'><p>Don't have an account</p><Link className='text-blue-500 underline' to='/register'>Register</Link></div>
          </div>
        </form>
      </div>
      </div>
